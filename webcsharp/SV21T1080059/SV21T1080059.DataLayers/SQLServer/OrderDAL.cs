    ...
    public class OrderDAL : BaseDAL, IOrderDAL
    {
        ...
        public Order? Get(int orderID)
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                data = connection.QueryFirstOrDefault<Order>(sql, param: parameters, commandType: System.Data.CommandType.Text);
                connection.Close();
            }
            return data;
        }
        ...
        public OrderDetail? GetDetail(int orderID, int productID)
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                data = connection.QueryFirstOrDefault<OrderDetail>(sql, param: parameters, commandType: System.Data.CommandType.Text);
                connection.Close();
            }
            return data;
        }
        ...
        public IList<Order> ListOfCustomer(int page = 1, int pageSize = 0, int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "", int customerID = 0)
        {
            List<Order> list = new List<Order>();
            if (!string.IsNullOrEmpty(searchValue))
                searchValue = "%" + searchValue + "%";
                ...
                var sql = @"with cte as
                    (
                        ...
                        from    Orders as o
                                ...
                                left join Employees as e on o.EmployeeID = e.EmployeeID
                                left join Shippers as s on o.ShipperID = s.ShipperID
                        where   c.CustomerID = @CustommerID
							and	(@Status = 0 or o.Status = @Status)
                            and (@FromTime is null or o.OrderTime >= @FromTime)
                            and (@ToTime is null or o.OrderTime <= @ToTime)
                            and (@SearchValue = N''
                                 or c.ContactName like @SearchValue
                                 or s.ShipperName like @SearchValue)
                    )
                    select * from cte
                    where (@PageSize = 0)
        ...
        public int Count(int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "", int customerID = 0)
        {
            int count = 0;
            if (!string.IsNullOrEmpty(searchValue))
                searchValue = "%" + searchValue + "%";
